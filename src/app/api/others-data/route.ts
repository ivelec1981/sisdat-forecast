...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get others data
    const othersData = await prisma.othersData.findMany({
      where,
      orderBy: { date: 'asc' },
    ...
    const summary = await prisma.othersData.aggregate({
      where,
      _count: { id: true },
      _avg: {
        enerProphet: true,
    ...
    const chartData = othersData.map(record => ({
    ...
    const yearlyData = othersData.reduce((acc, record) => {
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Error al obtener datos de otros sectores',
