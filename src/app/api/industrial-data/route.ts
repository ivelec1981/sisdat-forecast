...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get industrial data
    const industrialData = await prisma.industrialData.findMany({
      where,
      orderBy: { date: 'asc' },
    ...
    const summary = await prisma.industrialData.aggregate({
      where,
      _count: { id: true },
      _avg: {
        enerProphet: true,
    ...
    // Get companies
    const companies = await prisma.industrialData.groupBy({
      by: ['powerCompany'],
      _count: { powerCompany: true }
    });
    
    // Format data for charts
    const chartData = industrialData.map(record => ({
    ...
    const yearlyData = industrialData.reduce((acc, record) => {
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Error al obtener datos industriales',
