...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get commercial data
    const commercialData = await prisma.commercialData.findMany({
      where,
      orderBy: { date: 'asc' },
    ...
    const summary = await prisma.commercialData.aggregate({
      where,
      _count: { id: true },
      _avg: {
        enerProphet: true,
    ...
    const chartData = commercialData.map(record => ({
    ...
    const yearlyData = commercialData.reduce((acc, record) => {
