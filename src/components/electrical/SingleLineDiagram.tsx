...
import { Zap, Power, Activity, AlertCircle, TrendingUp, Info } from 'lucide-react';
...
export default function SingleLineDiagram() {
  // Interactive single-line diagram for Ecuador's National Interconnected System
  ...
  // Sistema Nacional Interconectado - Principales nodos
  ...
  // Calcular posición de línea con offset para evitar superposición
  ...
  // Obtener grosor de línea según capacidad
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
          ...
          <div className="text-2xl font-bold text-green-900">
            {systemMetrics.totalGeneration.toFixed(0)} MW
          </div>
          <div className="text-xs text-green-600 mt-1">
            de {systemMetrics.totalCapacity.toFixed(0)} MW disponibles
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-4 rounded-xl border border-blue-200">
          ...
          <div className="text-2xl font-bold text-blue-900">
            {systemMetrics.totalLoad.toFixed(0)} MW
          </div>
          <div className="text-xs text-blue-600 mt-1">
            {((systemMetrics.totalLoad / systemMetrics.totalGeneration) * 100).toFixed(1)}% de generación
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-violet-50 p-4 rounded-xl border border-purple-200">
          ...
          <div className="text-2xl font-bold text-purple-900">
            {systemMetrics.utilizationPercent.toFixed(1)}%
          </div>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 rounded-xl border border-amber-200">
          ...
          <div className="text-2xl font-bold text-amber-900">
            {systemMetrics.linesActive}/{systemMetrics.linesTotal}
          </div>
                ...
                return (
                  <g key={line.id}>
                    ...
                    <path
                      ...
                      strokeWidth={isSelected ? getLineWidth(line.voltage) + 2 : getLineWidth(line.voltage)}
                      strokeLinecap="round"
                    ...
                    {showFlow && showLabels && line.flow && (
                      <text
                        x={(buses.find(b => b.id === line.from)!.x + buses.find(b => b.id === line.to)!.x) / 2}
                        y={(buses.find(b => b.id === line.from)!.y + buses.find(b => b.id === line.to)!.y) / 2 - 10}
                        textAnchor="middle"
                    ...
                    {showLabels && (
                      <g>
                        <text
                          ...
                          y={bus.y - 20}
                          textAnchor="middle"
                          className="font-semibold fill-slate-900 pointer-events-none"
                          style={{ fontSize: '12px' }}
                        >
                          {bus.name}
                        </text>
                    ...
                    {viewMode === 'detailed' && bus.load && bus.capacity && (
                      <g>
                        ...
                        <rect
                          x={bus.x - 15}
                          y={bus.y + 15}
                          width={(bus.load / bus.capacity) * 30}
                          height="4"
                          fill={bus.load / bus.capacity > 0.9 ? '#EF4444' : '#10B981'}
                          rx="2"
                        />
                      </g>
                    )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-blue-900 font-bold">{bus.voltage} kV</p>
                    </div>
                    <div>
                      <span className="text-blue-700 font-medium">Tipo:</span>
                      <p className="text-blue-900 font-bold capitalize">{bus.type}</p>
                    </div>
                    <div>
                      <span className="text-blue-700 font-medium">Región:</span>
                      <p className="text-blue-900 font-bold">{bus.region}</p>
                    </div>
                    {bus.capacity && (
                      <div>
                        <span className="text-blue-700 font-medium">Capacidad:</span>
                        <p className="text-blue-900 font-bold">{bus.capacity} MW</p>
                      </div>
                    )}
                    {bus.load && (
                      <div>
                        <span className="text-blue-700 font-medium">Carga Actual:</span>
                        <p className="text-blue-900 font-bold">{bus.load} MW</p>
                      </div>
                    )}
                    {bus.load && bus.capacity && (
                      <div>
                        <span className="text-blue-700 font-medium">Utilización:</span>
                        <p className="text-blue-900 font-bold">{((bus.load / bus.capacity) * 100).toFixed(1)}%</p>
                      </div>
                    )}
                  </div>
                  ...
                  {generator && (
                    <div className="mt-3 pt-3 border-t border-blue-200">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="text-blue-700 font-medium">Tipo:</span>
                          <p className="text-blue-900 font-bold capitalize">{generator.type === 'hydro' ? 'Hidráulica' : generator.type}</p>
                        </div>
                        <div>
                          <span className="text-blue-700 font-medium">Capacidad:</span>
                          <p className="text-blue-900 font-bold">{generator.capacity} MW</p>
                        </div>
                        <div>
                          <span className="text-blue-700 font-medium">Generación:</span>
                          <p className="text-blue-900 font-bold">{generator.generation} MW</p>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              return (
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <span className="text-blue-700 font-medium">Voltaje:</span>
                      <p className="text-blue-900 font-bold">{line.voltage} kV</p>
                    </div>
                    <div>
                      <span className="text-blue-700 font-medium">Capacidad:</span>
                      <p className="text-blue-900 font-bold">{line.capacity} MW</p>
                    </div>
                    <div>
                      <span className="text-blue-700 font-medium">Flujo Actual:</span>
                      <p className="text-blue-900 font-bold">{line.flow} MW</p>
                    </div>
                    <div>
                      <span className="text-blue-700 font-medium">Carga:</span>
                      <p className={`font-bold ${
                        loadPercent > 90 ? 'text-red-600' :
                        loadPercent > 75 ? 'text-orange-600' :
                        'text-green-600'
                      }`}>
                    ...
                    <div>
                      ...
                      <p className={`font-bold capitalize ${
                        line.status === 'active' ? 'text-green-600' :
                        line.status === 'overload' ? 'text-red-600' :
                        'text-orange-600'
                      }`}>
                        {line.status === 'active' ? 'Activa' :
                         line.status === 'overload' ? 'Sobrecarga' :
                         'Mantenimiento'}
                      </p>
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-700">Generación</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full bg-purple-500 border-2 border-white"></div>
                <span className="text-sm text-slate-700">Transmisión</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full bg-amber-500 border-2 border-white"></div>
                <span className="text-sm text-slate-700">Distribución</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full bg-red-500 border-2 border-white"></div>
                <span className="text-sm text-slate-700">Carga</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-700">Normal (&lt; 75%)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-1 bg-orange-500 rounded"></div>
                <span className="text-sm text-slate-700">Carga Alta (75-90%)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-1 bg-red-500 rounded"></div>
                <span className="text-sm text-slate-700">Sobrecarga (&gt; 90%)</span>
              </div>
              <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-700">230 kV (Transmisión)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-1 bg-slate-600 rounded"></div>
                <span className="text-sm text-slate-700">138 kV (Sub-transmisión)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-0.5 bg-slate-500 rounded"></div>
                <span className="text-sm text-slate-700">&lt; 69 kV (Distribución)</span>
              </div>
            </div>
          </div>
        </div>
