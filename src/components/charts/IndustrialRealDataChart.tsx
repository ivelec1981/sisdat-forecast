...
import { Calendar, TrendingUp, Zap, BarChart3, Factory } from 'lucide-react';
import { useIndustrialData } from '@/hooks/useIndustrialData';

interface IndustrialRealDataChartProps {
  title?: string;
  height?: number;
}

export default function IndustrialRealDataChart({ 
  title = "Datos Reales - CNEL-Guayas Los Ríos (Sector Industrial)", 
  height = 400 
}: IndustrialRealDataChartProps) {
  const [selectedYear, setSelectedYear] = useState<number>(2024);
  const [viewMode, setViewMode] = useState<'energy' | 'power'>('energy');
  
  const { data: industrialData, summary, loading } = useIndustrialData("CNEL-Guayas Los Ríos");
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          <Factory className="w-5 h-5 text-amber-500 animate-pulse" />
    ...
    return (
      ...
      >
        ...
        <div className="text-center text-slate-500 py-8">
          No hay datos industriales disponibles
        </div>
      </motion.div>
    );
  }
  ...
  const availableYears = Array.from(new Set(industrialData.map(d => d.year))).sort((a, b) => b - a);
  
  // Filtrar datos por año seleccionado
  const yearData = industrialData
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div className="relative">
            ...
            <select
              ...
              className="pl-10 pr-4 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-colors"
            >
      ...
      >
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
            <defs>
              <linearGradient id="prophetGradientInd" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#3B82F6" stopOpacity={0.3}/>
                <stop offset="95%" stopColor="#3B82F6" stopOpacity={0}/>
              </linearGradient>
              <linearGradient id="gruGradientInd" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#10B981" stopOpacity={0.3}/>
                <stop offset="95%" stopColor="#10B981" stopOpacity={0}/>
              </linearGradient>
              <linearGradient id="wavenetGradientInd" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#F59E0B" stopOpacity={0.3}/>
                <stop offset="95%" stopColor="#F59E0B" stopOpacity={0}/>
              </linearGradient>
              <linearGradient id="combinedGradientInd" x1="0" y1="0" x2="0" y2="1">
      ...
      <motion.div 
        ...
        className="mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-700"
      >
